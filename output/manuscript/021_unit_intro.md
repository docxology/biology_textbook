...
```
<!-- alt: Graph showing unit IV concept map — Molecular Genetics. Dark = DNA; purple = regulation; orange = mutations; red = editing technology. -->

*Unit IV concept map — Molecular Genetics. Dark = DNA; purple = regulation; orange = mutations; red = editing technology.*
...
- **DNA replication** directly links to Unit V (meiosis and crossing-over in \cref{sec:unit_V_chromosomal_inheritance}) and Unit VI (molecular clock calculations using sequence divergence).
- **Gene regulation** (operons, TFs, miRNA) reappears in Unit VIII (plant hormone signal transduction) and Unit IX (endocrine control of gene expression via steroid receptors).
- **Mutations and DNA repair** are essential context for Unit VI (evolution as mutation + selection) and oncogene/tumour-suppressor biology in Unit II and Unit IX.
- **CRISPR-Cas9** appears in clinical connection boxes throughout — gene therapy (Unit V), antibiotic resistance (Unit VII), and plant engineering (Unit VIII).
