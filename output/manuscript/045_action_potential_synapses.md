...
Alan Hodgkin and Andrew Huxley recorded action potentials from the **squid giant axon** (diameter ~0.5 mm) using voltage clamp (Hodgkin & Huxley, 1952, *J Physiol* -- Nobel Prize 1963). Each ionic current is driven by the difference between $V$ and that ion's Nernst equilibrium potential, the gradient-dependent values of which are plotted in \cref{fig:unit_IX_nernst_potentials}. They described membrane current as:
