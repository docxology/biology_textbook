...
```
<!-- alt: Graph showing unit III concept map — Energy and Metabolism. Orange = ATP currency; red = oxidative phosphorylation; green = photosynthesis; purple = ATP synthase. -->

*Unit III concept map — Energy and Metabolism. Orange = ATP currency; red = oxidative phosphorylation; green = photosynthesis; purple = ATP synthase.*
...
- **ATP and free energy** introduced here underpin most driven processes in Unit IV (DNA replication, transcription, translation), Unit VIII (active transport in phloem), and Unit IX (Na⁺/K⁺-ATPase in neurons).
- **The TCA cycle as metabolic hub** is integrated in \cref{sec:unit_III_metabolic_integration} and connects to amino acid catabolism (Unit IV), fatty acid oxidation, and nucleotide synthesis.
- **Proton-motive force** is the conceptual bridge between mitochondrial ATP synthesis (this unit) and chloroplast ATP synthesis (\cref{sec:unit_III_photosynthesis}), and between bacterial physiology (Unit VII) and the evolution of eukaryotes (Unit II, endosymbiosis).
- **Warburg effect** (cancer cells performing aerobic glycolysis) appears in Unit II (cell signalling oncogenes), Unit IV (genomic instability), and the clinical connection sections throughout.
