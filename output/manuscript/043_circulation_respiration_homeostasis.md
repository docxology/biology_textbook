...
Calculate the rate of O$_2$ diffusion across the alveolar-capillary barrier using Fick's law with these values:
- $D_{O_2}$ (in tissue at 37°C) $\approx 1.5 \times 10^{-5}$ cm$^2$/s
- $A$ = 70 m$^2$ = $70 \times 10^4$ cm$^2$
- $\Delta P_{O_2}$ = alveolar $PO_2$ 100 mmHg $-$ venous $PO_2$ 40 mmHg = 60 mmHg $= 60 \times 133.3$ Pa $\approx 8{,}000$ Pa; for gas diffusion use $\Delta P$ in mmHg directly with a tissue-specific coefficient
- $T$ = 0.5 μm $= 0.5 \times 10^{-4}$ cm

Using Krogh's diffusion constant $K_{O_2} \approx 1.73 \times 10^{-9}$ mL O$_2$/(cm$\cdot$s$\cdot$mmHg):

\begin{equation}
\dot{V}_{O_2} = K_{O_2} \cdot A \cdot \frac{\Delta P}{T} = 1.73 \times 10^{-9} \times 70 \times 10^4 \times \frac{60}{0.5 \times 10^{-4}}
\label{eq:circulation_respiration_homeostasis_14}
\end{equation}

\begin{equation}
= 1.73 \times 10^{-9} \times 7 \times 10^5 \times 1.2 \times 10^6 \approx 1.45\;\text{mL O}_2/\text{s} = 87\;\text{mL O}_2/\text{min}
\label{eq:circulation_respiration_homeostasis_15}
\end{equation}

At rest, whole-body O$_2$ consumption is ~250 mL/min — so the diffusing capacity, even under resting conditions, is ample. During maximal exercise, increased cardiac output, capillary recruitment, and a wider $\Delta P$ (tissues extract more O$_2$, reducing venous $PO_2$ to ~20 mmHg) push diffusion to the limit in elite athletes.
