...
> (Slamon et al., 1987, *Science*). In normal cells, HER2 is transiently activated when it binds
...
> trastuzumab therapy approximately doubled 5-year survival. Every step in this therapeutic triumph
> required understanding how cells receive, transduce, and amplify molecular signals — precisely the
> subject of this chapter.
>
> *Primary source: Slamon, D. J. et al. (1987). Human breast cancer: Correlation of relapse and survival with amplification of the HER-2/neu oncogene. Science, 235(4785), 177–182.*
...
1. **Amplification:** One activated receptor activates multiple G proteins; each G protein activates multiple adenylyl cyclase molecules; each adenylyl cyclase produces many cAMP molecules. A single epinephrine molecule can trigger release of ~10$^8$ glucose molecules from glycogen (10$^6$-fold amplification).
...
GPCRs are the largest receptor superfamily (~800 in humans; ~34% of approved drug targets). They have 7 transmembrane alpha-helices, an extracellular N-terminus, and an intracellular C-terminus. Binding of ligand activates the heterotrimeric G protein (G-alpha, G-beta-gamma):
...
Hormone (e.g., epinephrine) → beta$_2$-adrenergic receptor → G$_s$ → **adenylyl cyclase** activated → ATP → **cAMP** → **Protein Kinase A (PKA)** → phosphorylates multiple downstream targets (glycogen phosphorylase kinase, CREB [**transcription**](#gl:transcription) factor, ion channels, etc.)
...
- **Connections:** See Unit IV (growth-factor-driven transcription), Unit V (tumour suppressor [**allele**](#gl:allele)s), Unit IX (endocrine first messengers).
