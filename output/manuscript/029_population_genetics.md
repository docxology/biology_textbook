...
If you are reading for the core Mendelian-genetics course, prioritise the Hardy-Weinberg model and the four evolutionary forces. The genome-scale extensions connect population genetics to molecular evolution (Unit VI) and are helpful but optional.
...
### Forensic and Population-Inference Boundaries

Population-genetic statistics are summaries of samples, not identities. A PCA cluster, ancestry proportion, or $F_{ST}$ estimate depends on the reference panel, marker set, filtering, and demographic model; changing any of these can shift the interpretation. In forensic or outbreak settings, phylogenetic closeness can support a transmission hypothesis, but direction and responsibility require timing, sampling density, chain-of-custody, and plausible exposure pathways. Good practice reports the null model, the comparison database, the uncertainty interval, and the alternative histories still compatible with the data.
...
- **Connections:** See Unit VI (tree thinking and substitution models), Unit X (metapopulation structure), Unit VII (microbial effective population sizes).
