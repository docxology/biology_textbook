...
```
<!-- alt: Graph showing unit X concept map — Ecology. Green = population; blue = community; orange = ecosystem; red = conservation. -->

*Unit X concept map — Ecology. Green = population; blue = community; orange = ecosystem; red = conservation.*
...
- **Population models** (logistic growth, age structure) use the same differential equation framework as Unit VII (SIR epidemiological models) — ecology and epidemiology are mathematically unified.
- **Species diversity indices** (Shannon $H'$, Simpson $D$) are computed using tools from Unit VII (microbial communities) and help evaluate ecosystem health.
- **Nutrient cycling** (nitrogen, carbon, phosphorus) directly ties to Unit III (carbon fixation by photosynthesis) and Unit VII (microbial decomposers, nitrogen fixation by bacteria).
- **Climate change impacts** on biomes connect to Unit VIII (plant responses to drought, CO₂ enrichment) and Unit VI (evolution of species under selection by climate).
- **Conservation biology** uses evolutionary genetics (Unit VI: effective population size, genetic rescue) and macroecology (island biogeography, keystone species).
