...
def build_grid(book_toc) -> str:
    ...
    for chapter in book_toc.chapters:
        ...
        rows.append(
            f"| {unit.name_ref} | {chapter.grid_number} | {chapter.name_ref} | {c.difficulty_label} | "
            f"{c.reading_time_min} min | {c.lecture_time_min} min |"
        )
    ...
    rows.append(
        f"| | | **Totals** | | **{totals_reading} min ({hours} h)** | "
        f"**{totals_lecture} min ({lecture_hours} h)** |"
    )
    return "\n".join(rows)
