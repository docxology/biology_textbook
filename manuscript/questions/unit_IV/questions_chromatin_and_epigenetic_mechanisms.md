...
<!-- SOLUTION
**Answer (Q29, Application).** (a) Because lncRNA sequences evolve rapidly, simple cross-species conservation cannot identify functional ones and model organisms may lack orthologues. (b) Knockdown or deletion can perturb the act of transcription or an underlying enhancer/DNA element rather than the RNA product itself, confounding interpretation. (c) Lacking ORFs and conserved domains, function must be inferred from structure and interactions. A systematic approach: combine RNA-targeting knockdown (ASOs/CRISPRi versus locus deletion controls), rescue with exogenous transcript, map RNA–chromatin/protein interactions (CHART/ChIRP, RAP), and test specific structural motifs by targeted mutation. See \cref{sec:unit_IV_chromatin_and_epigenetic_mechanisms}.
SOLUTION -->
