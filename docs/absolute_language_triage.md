...
Current triage rule is recorded in `manuscript/quality_advisories.yaml`.
...
| Needed qualifier | Edit when the line turns an empirical trend, scenario, treatment effect, or current count into a universal claim. Fast-moving examples belong in `manuscript/current_claims.yaml`. If the same advisory remains with `needs_qualifier`, the audit fails. |
