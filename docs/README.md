...
- **Figures** (`src/visualization/plots.py`, 32 generators; **`src/visualization/cvd.py`** for colorvision–friendly defaults tied to `config.yaml` → `accessibility.color_blindness_safe`) and **diagrams** (`src/mermaid/`, 24 registered diagrams plus 196 inline Mermaid fences, optional `mmdc`)
