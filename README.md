...
**Build health:** run `uv run pytest tests/ --cov=src --cov-fail-under=90` from this directory for test count and coverage; the current filesystem has 31 Python files under `scripts/` and 27 `test_*.py` modules under `tests/`. `references.bib`, `glossary.md`, lab computation snippets, question-bank answers, current-claim ledger entries, quality-advisory triage, assessment metadata, visual-contract manifest findings, textbook-quality audit findings, and script hygiene are kept closed by tests. The question-bank gate rejects generated-style solution scaffolds such as `Expected reasoning:`, `Key answer:`, and `Mechanistic answer:`. There are 18 `plot_*` figure generators, 24 registered Mermaid diagrams in `src/`, and 192 inline Mermaid fences in the manuscript. Regenerate the combined PDF to refresh file size and layout.

## Location

This tree is currently under **`projects_in_progress/biology_textbook/`**. `resolve_project_root` prefers `projects/biology_textbook/` when that active tree has `src/`, `tests/`, `scripts/`, and `manuscript/`; otherwise it resolves the same name under `projects_in_progress/`. Pipeline entry points use `--project biology_textbook` from the template repository root.
...
uv sync
uv run pytest tests/ --cov=src --cov-fail-under=90 -v     # run from this directory for the 90% gate
...
uv run python scripts/audit_visual_contracts.py --check   # visual manifest and alt/caption contract
```
...
├── src/visualization/ # __init__.py (ALL_FIGURE_GENERATORS, 18 plots) + cvd.py (palette)
├── scripts/           # 31 Python files — see [AGENTS.md](AGENTS.md) (core build + content maintenance)
├── tests/             # 27 test modules, ≥90% line+branch on `src/`, no mocks
